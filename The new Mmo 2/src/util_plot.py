...
from model_solution import SolutionData

def _get_unique_nodes_from_fe_schedule_dop(schedule: tuple[dict]) -> List[int]:
...
def plot_solution_visualization_dop(solution_data: SolutionData, save_dir: str):
    ...
    for i, fe_route in enumerate(solution_data.fe_routes):
        ...
        for se_idx in fe_route.serviced_se_route_indices:
            # Lấy satellite_id từ SE route tương ứng
            sat_id = solution_data.se_routes[se_idx].satellite_id
            satellite_to_color_map[sat_id] = color
    ...
    # Vẽ SE Routes
    for se_route in solution_data.se_routes:
        color = satellite_to_color_map.get(se_route.satellite_id, 'gray')
        path_coords = [
            (problem.node_objects[nid % problem.total_nodes].x, 
             problem.node_objects[nid % problem.total_nodes].y) 
            for nid in se_route.nodes_id
        ]
    ...
    for i, fe_route in enumerate(solution_data.fe_routes):
        color = fe_route_colors(i)
        path_node_ids = _get_unique_nodes_from_fe_schedule_dop(fe_route.schedule)
    ...
    # Tính lại cost tổng để hiển thị trên tiêu đề
    # (Dùng hàm tính đơn giản tổng dist cho hiển thị)
    total_dist = sum(r.total_dist for r in solution_data.fe_routes) + sum(r.total_dist for r in solution_data.se_routes)
    
    ax.set_title(f"2E-VRP Solution (Total Distance: {total_dist:.2f})", fontsize=18)
    ax.set_aspect('equal')
    plt.tight_layout()
...
def plot_alns_history(run_history: Dict, op_history: Dict, save_dir: str):
    # Hàm này không phụ thuộc vào SolutionData, chỉ phụ thuộc vào dict lịch sử, nên giữ nguyên logic cũ
